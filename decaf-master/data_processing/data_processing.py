...
def process_videoReceiveStream_log(path):
	...
	# 新增两行，确保在向 data_dict 添加新的 ssrc 时，确保 ssrc 在字典中存在。
	# if ssrc not in data_dict:
	# 	data_dict[ssrc] = {}

	for tag in tags_list:

		# 新增两行，以确保 'time_ms' 存在于 data_dict[ssrc] 中：
		# if tag == 'time_ms' and tag not in data_dict[ssrc]:
		# 	data_dict[ssrc][tag] = []
		# print(data_dict)
		# print("ssrc:", ssrc)
		# print("data_dict[ssrc]:", data_dict[ssrc])
		# print("data_dict[ssrc][tag]:", data_dict[ssrc][tag])
							...
							if lk == "packetsLost" and media_type in ['audio', 'video']:
								if lk not in list(data_dict[l_type][l_id].keys()):
									...
									if dict_line[lk] < 0:
										...
										if loss_d[media_type]['previous_loss'] < 0 and loss_d[media_type][
											'previous_loss'] < dict_line[lk]:
											loss_d[media_type]['correct_loss'] += dict_line[lk] - loss_d[media_type][
												'previous_loss']

										elif loss_d[media_type]['previous_loss'] >= 0 and loss_d[media_type][
											'previous_loss'] < dict_line[lk]:
											loss_d[media_type]['correct_loss'] += dict_line[lk] - loss_d[media_type][
												'previous_loss']
...
def process_game_recording(path, param_dict):
	...
	if video_orig != "":
		...
		if not os.path.exists(path + "video_cropped.mkv"):
			cmd = "ffmpeg -i " + path + video_orig + " -filter:v " + "'" + param_dict["video_crop_param"][
				game + "_" + platform] + "' " + path + "video_cropped.mkv"
			print("\nCroping video: ", cmd)
			os.system(cmd)
			print("Cropped")
		else:
